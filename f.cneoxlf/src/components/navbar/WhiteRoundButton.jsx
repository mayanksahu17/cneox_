...
const WhiteRoundButton = ({ text = "Click Me!", className = "" }) => {
  ...
  return (
    <button
      ref={buttonRef}
      className={`relative border-2 border-solid border-[#ffffff] px-5 py-2 rounded-3xl text-[#ffffff] overflow-hidden hover:text-white hover:border-[#4CAF50] bg-yellow-500 z-0 ${className}`}
    ...
    >
      <div
        ref={divRef}
        className="absolute w-[200px] h-[150px] bg-[#ffffff] left-0 top-0 wrapperElement -z-10 pointer-events-none rounded-[50%]"
      ></div>
